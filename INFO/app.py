...
from security_integration import init_security, secure_login_required, secure_admin_required
# Note: consent_privacy_module and related modules can be added later if needed
# from consent_privacy_module import consent_manager, access_logger, notification_manager
# from consent_privacy_routes import consent_bp, require_consent
# from doctor_patient_views import doctor_patient_bp
...
try:
    from blockchain_utils import (
        register_doctor_onchain, register_patient_onchain, add_record_onchain,
        is_blockchain_connected, get_blockchain_stats, get_all_record_metadata,
        get_registered_roles, sync_user_to_blockchain, compute_record_hash,
        verify_record_onchain, get_role_onchain
    )
    BLOCKCHAIN_AVAILABLE = True
except Exception as e:
    BLOCKCHAIN_AVAILABLE = False
    print(f"Blockchain utils not loaded: {e}")
...
# Load contract config once at startup
...
try:
    ...
    app.config['CONTRACT_ADDRESS'] = _contract['contractAddress']
    app.config['CONTRACT_ABI']     = _contract['abi']
except Exception as e:
    app.logger.warning(f"Could not load contract config: {e}")
    app.config['CONTRACT_ADDRESS'] = ''
    app.config['CONTRACT_ABI']     = []
 
@app.context_processor
def inject_contract():
    return {
        'CONTRACT_ADDRESS': app.config.get('CONTRACT_ADDRESS', ''),
        'CONTRACT_ABI': app.config.get('CONTRACT_ABI', [])
    }
...
def before_request():
    ...
    allowed_endpoints = [
        'login', 'register', 'admin_login', 'qr_page', 'show_qr', 'two_factor',
    ...
    if request.method == 'POST':
        try:
            ...
            if not re.match(r'^0x[a-fA-F0-9]{40}$', wallet_address):
               flash('Invalid wallet address. Must be a valid Ethereum address (0x...).', 'danger')
               log_action(username or 'anonymous', 'register_failed', 'Registration failed: Invalid wallet address.')
               return redirect(url_for('register'))
            ...
            try:
                ...
                if cur.fetchone()[0] > 0:
                    ...
                    return redirect(url_for('register'))
                cur.execute("SELECT COUNT(*) FROM users WHERE wallet_address = %s", (wallet_address,))
                if cur.fetchone()[0] > 0:
                    flash('Wallet address already exists.', 'danger')
                    log_action(username or 'anonymous', 'register_failed', 'Registration failed: Wallet address already exists.')
                    return redirect(url_for('register'))
                ...
                if role == 'doctor':
                    ...
                    cur.execute(
                        """
                        INSERT INTO users (username, email, password, 2fa_secret, role, specialization, license_number, doctor_id_image, private_key, public_key, wallet_address)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                        """,
                        (username, email, hashed_pw, totp_secret, role, specialization, license_number, doctor_id_image_bytes, private_pem, public_pem, wallet_address)
                    )
                else:
                    cur.execute(
                        """
                        INSERT INTO users (username, email, password, 2fa_secret, role, wallet_address)
                        VALUES (%s, %s, %s, %s, %s, %s)
                        """,
                        (username, email, hashed_pw, totp_secret, role, wallet_address)
                    )
                mysql.connection.commit()

                # Auto-register on blockchain
                if BLOCKCHAIN_AVAILABLE and wallet_address and role in ('doctor', 'patient'):
                    try:
                        result = sync_user_to_blockchain(wallet_address, role)
                        if result['success']:
                            app.logger.info(f"Auto-registered {username} ({wallet_address}) as {role} on blockchain.")
                        else:
                            app.logger.warning(f"Blockchain auto-registration failed for {username}: {result.get('error')}")
                    except Exception as bc_err:
                        app.logger.warning(f"Blockchain registration error for {username}: {bc_err}")
...
def admin_login():
    """Dedicated admin login route — validates credentials + checks admin role."""
    if is_logged_in():
    ...
    if request.method == 'POST':
        ...
        if not user:
            log_action(username, 'admin_login_failed', f'Admin login failed: user {username} not found.')
            flash('Invalid admin credentials.', 'danger')
            return redirect(url_for('login'))

        # Check password
        if not bcrypt.check_password_hash(user[3], password):
            log_action(username, 'admin_login_failed', f'Admin login failed: wrong password for {username}.')
            flash('Invalid admin credentials.', 'danger')
            return redirect(url_for('login'))

        # Check role is admin
        if user[4] != 'admin':
            log_action(username, 'admin_login_denied', f'Non-admin user {username} tried admin login.')
            flash('Access denied. Administrator privileges required.', 'danger')
            return redirect(url_for('login'))

        # Credentials valid + role is admin → proceed to 2FA
        session['pre_2fa_user'] = user[1]
        log_action(username, 'admin_login_password_success', f'Admin {username} password verified. Proceeding to 2FA.')
        return redirect(url_for('two_factor'))

    # GET request redirects to login page
    return redirect(url_for('login'))
...
def two_factor():
    ...
    if request.method == 'POST':
        ...
        if totp.verify(token):
            ...
            # Load wallet_address into session — independent, always runs
            try:
                 cur = mysql.connection.cursor()
                 cur.execute("SELECT wallet_address FROM users WHERE username = %s", (username,))
                 wallet_row = cur.fetchone()
                 cur.close()
                 if wallet_row and wallet_row[0]:
                     session['wallet_address'] = wallet_row[0]
            except Exception:
                 pass
...
def admin_dashboard():
    ...
    try:
        ...
        # Construct user query — include wallet_address
        user_query = "SELECT id, username, email, role, wallet_address FROM users"
        user_params = []
        ...
        # Get medical records stats with blockchain info
        try:
        ...
        try:
            cur.execute("SELECT COUNT(*) FROM medical_records WHERE record_hash IS NOT NULL")
            hashed_records = cur.fetchone()[0]
        except Exception:
            hashed_records = 0

        try:
            cur.execute("SELECT COUNT(*) FROM medical_records WHERE blockchain_record_id IS NOT NULL")
            onchain_records = cur.fetchone()[0]
        except Exception:
            onchain_records = 0
        ...
        if BLOCKCHAIN_AVAILABLE:
            try:
                ...
                app.logger.warning(f"Could not get blockchain stats: {e}")

        # Get on-chain roles for all users with wallets
        onchain_roles = {}
        if BLOCKCHAIN_AVAILABLE and blockchain_stats.get('connected'):
            try:
                wallets = [u[4] for u in users if u[4]]
                onchain_roles = get_registered_roles(wallets)
            except Exception as e:
                app.logger.warning(f"Could not get on-chain roles: {e}")

        # Build all_users list for wallet assignment dropdown
        all_users_list = []
        for u in users:
            all_users_list.append({
                'id': u[0], 'username': u[1], 'role': u[3],
                'wallet': u[4] if len(u) > 4 else None
            })
...
@app.route('/admin/blockchain/register', methods=['POST'])
@admin_required
def admin_blockchain_register():
    """Assign a wallet to a user in DB and register it on the blockchain."""
    wallet = request.form.get('wallet_address', '').strip()
    user_id = request.form.get('user_id', '').strip()

    if not wallet or not user_id:
        flash('User and wallet address are required.', 'danger')
        return redirect(url_for('admin_dashboard'))
    ...
    if not BLOCKCHAIN_AVAILABLE:
        flash('Blockchain utils not available.', 'danger')
        return redirect(url_for('admin_dashboard'))
    ...
    try:
        ...
        if role not in ('doctor', 'patient'):
            flash(f'Cannot register admin wallets on-chain. Only doctor/patient roles.', 'warning')
            # Still save wallet to DB for admin
            cur.execute("UPDATE users SET wallet_address = %s WHERE id = %s", (wallet, user_id))
            mysql.connection.commit()
            flash(f'Wallet saved to DB for {username} but not registered on-chain (admin role).', 'info')
            return redirect(url_for('admin_dashboard'))
        ...
        result = sync_user_to_blockchain(wallet, role)
        if result['success']:
            if result.get('already_registered'):
                flash(f'Wallet saved to DB for {username}. Already registered as {role} on-chain.', 'info')
            else:
                flash(f'Wallet saved to DB and registered as {role} on blockchain for {username}. TX: {result["tx_hash"][:16]}...', 'success')
            log_action(session['username'], 'blockchain_register',
                       f'Assigned wallet {wallet} to user {username} (#{user_id}) and registered as {role} on blockchain.')
        else:
            flash(f'Wallet saved to DB but blockchain registration failed: {result["error"]}', 'warning')
    except Exception as e:
...
@app.route('/admin/blockchain/sync', methods=['POST'])
@admin_required
def admin_blockchain_sync():
    """Batch-sync all DB users (doctors/patients) to the blockchain."""
    if not BLOCKCHAIN_AVAILABLE:
        flash('Blockchain utils not available.', 'danger')
        return redirect(url_for('admin_dashboard'))
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT id, username, role, wallet_address FROM users WHERE wallet_address IS NOT NULL AND wallet_address != '' AND role IN ('doctor', 'patient')")
        ...
        for user in users_to_sync:
            uid, uname, urole, uwallet = user
            try:
                result = sync_user_to_blockchain(uwallet, urole)
                if result['success']:
                    if result.get('already_registered'):
                        skipped += 1
                    else:
                        synced += 1
                        app.logger.info(f"Synced {uname} ({uwallet}) as {urole} to blockchain.")
                else:
                    failed += 1
                    app.logger.warning(f"Failed to sync {uname}: {result['error']}")
            except Exception as e:
                failed += 1
                app.logger.warning(f"Error syncing {uname}: {e}")

        flash(f'Blockchain sync complete: {synced} registered, {skipped} already synced, {failed} failed.', 'success')
        log_action(session['username'], 'blockchain_sync',
                   f'Batch blockchain sync: {synced} registered, {skipped} skipped, {failed} failed.')

    except Exception as e:
        flash(f'Blockchain sync error: {str(e)}', 'danger')
    finally:
        if cur:
            cur.close()
...
@app.route('/admin/blockchain/records')
@admin_required
def admin_blockchain_records_api():
    """JSON API: return all on-chain record metadata."""
    if not BLOCKCHAIN_AVAILABLE:
        return jsonify({'error': 'Blockchain not available'}), 503

    try:
        records = get_all_record_metadata()
        return jsonify({'records': records, 'count': len(records)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/admin/blockchain/verify/<int:record_id>')
@admin_required
def admin_blockchain_verify(record_id):
    """Verify a DB medical record against the blockchain."""
    if not BLOCKCHAIN_AVAILABLE:
        return jsonify({'error': 'Blockchain not available'}), 503
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute("""
            SELECT mr.id, mr.patient_id, mr.doctor_id, mr.diagnosis, mr.treatment,
                   mr.record_hash, mr.blockchain_record_id, mr.blockchain_tx
            FROM medical_records mr WHERE mr.id = %s
        """, (record_id,))
        row = cur.fetchone()
        ...
        computed_hash = compute_record_hash(row[1], row[2], row[3], row[4] or '')
        result['computed_hash'] = computed_hash
        result['hash_verified'] = (row[5] == computed_hash) if row[5] else False

        # Verify on blockchain
        if row[6]:
            chain_result = verify_record_onchain(int(row[6]))
            result['on_chain'] = chain_result.get('exists', False)
            result['chain_data'] = chain_result
...
@app.route('/admin/blockchain/hash-records', methods=['POST'])
@admin_required
def admin_hash_records():
    """Compute and store SHA-256 hashes for all medical records that don't have one."""
    cur = None
    try:
        cur = mysql.connection.cursor()
        cur.execute("""
            SELECT id, patient_id, doctor_id, diagnosis, treatment
        ...
        count = 0
        for row in unhashed:
            rid, pid, did, diag, treat = row
            record_hash = compute_record_hash(pid, did, diag, treat or '')
            cur.execute("UPDATE medical_records SET record_hash = %s WHERE id = %s",
                        (record_hash, rid))
            count += 1

        mysql.connection.commit()
        flash(f'Computed and stored hashes for {count} medical records.', 'success')
        log_action(session['username'], 'hash_records',
                   f'Hashed {count} medical records for blockchain integrity.')

    except Exception as e:
        flash(f'Error hashing records: {str(e)}', 'danger')
    finally:
        if cur:
            cur.close()
...
def admin_add_user():
    if request.method == 'POST':
        try:
            ...
            if cursor.fetchone():
                flash('Wallet address already registered.', 'danger')
                return redirect(url_for('admin_add_user'))
            ...
            cursor.execute('''
                INSERT INTO users (
                    ...
                    phone, date_of_birth, gender, blood_type,
                    emergency_contact, emergency_phone, specialization,
                    license_number, 2fa_secret, wallet_address, created_at
                ) VALUES (
                    %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                    %s, %s, %s, %s, %s, %s, NOW()
                )
            ''', (
                ...
                phone, date_of_birth, gender, blood_type,
                emergency_contact, emergency_phone, specialization,
                license_number, twofa_secret, wallet_address
            ))
            mysql.connection.commit()

            # Auto-register on blockchain
            try:
                from blockchain_utils import register_doctor_onchain, register_patient_onchain
                if role == 'doctor':
                    register_doctor_onchain(wallet_address)
                    app.logger.info(f"Registered doctor {username} on blockchain.")
                elif role == 'patient':
                    register_patient_onchain(wallet_address)
                    app.logger.info(f"Registered patient {username} on blockchain.")
            except Exception as e:
                app.logger.warning(f"Blockchain registration failed for {username}: {e}")
                flash(f'User created but blockchain registration failed: {e}', 'warning')

            log_action(session['username'], 'user_created',
                       f'Created new {role} account: {username}')
            flash(f'Successfully created new {role} account and registered on blockchain.', 'success')
            return redirect(url_for('admin_dashboard'))
        ...
        if request.method == 'POST':
            ...
            if ext in image_extensions:
                try:
                    ...
                    # Model file missing - silently skip analysis, don't set tumor_detected
                    app.logger.warning(f"MRI model not available - skipping analysis. File uploaded successfully.")
                    tumor_detected = None
                    confidence = None
                    analysis_date = None
                except Exception as e:
                    # Analysis failed for other reasons - silently skip analysis
                    app.logger.warning(f"MRI analysis error (skipping): {e}")
                    tumor_detected = None
                    confidence = None
                    analysis_date = None
...
def _watermark_image(input_path: str, output_path: str, text: str = 'SECURE HEALTH'):
    try:
        with Image.open(input_path).convert('RGBA') as base:
            ...
            # Use textbbox instead of deprecated textsize (Pillow 10+)
            try:
                ...
                try:
                    ...
                    # If textsize also fails, estimate size
        ...
        # Fallback: copy file without watermark so analysis can still happen
        try:
            import shutil
            shutil.copyfile(input_path, output_path)
            app.logger.info(f"Copied file without watermark: {output_path}")
        except Exception as copy_error:
            app.logger.error(f"Failed to copy file after watermark error: {copy_error}")
            raise
...
def upload_radiology_in_folder(folder):
    ...
    cur = mysql.connection.cursor()
    try:
        cur.execute("SELECT id, first_name, last_name FROM users WHERE username = %s", (session['username'],))
        patient = cur.fetchone()
        patient_id = patient[0]
        patient_name = f"{patient[1]} {patient[2]}" if patient[1] and patient[2] else session['username']
        
        base_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'radiology', str(patient_id))
        user_dir = os.path.join(base_dir, folder)
        os.makedirs(user_dir, exist_ok=True)

        # Handle doctor assignment (separate from file upload)
        if request.method == 'POST' and 'assign_doctor' in request.form:
            assigned_doctor_id = request.form.get('assigned_doctor_id')
            if assigned_doctor_id:
                try:
                    assigned_doctor_id = int(assigned_doctor_id)
                    # Ensure radiology_folder_assignments table exists
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS radiology_folder_assignments (
                            id INT AUTO_INCREMENT PRIMARY KEY,
                            patient_id INT NOT NULL,
                            ...
                            FOREIGN KEY (patient_id) REFERENCES users(id) ON DELETE CASCADE,
                            FOREIGN KEY (assigned_doctor_id) REFERENCES users(id) ON DELETE CASCADE,
                            UNIQUE KEY unique_assignment (patient_id, folder),
                            INDEX idx_folder_assignment (folder, assigned_doctor_id)
                        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
                    """)
                    mysql.connection.commit()
                    ...
                    cur.execute("""
                        SELECT assigned_doctor_id FROM radiology_folder_assignments 
                        WHERE patient_id = %s AND folder = %s
                    """, (patient_id, folder))
                    ...
                    if existing:
                        ...
                        cur.execute("""
                            UPDATE radiology_folder_assignments 
                            SET assigned_doctor_id = %s, assigned_at = CURRENT_TIMESTAMP
                            WHERE patient_id = %s AND folder = %s
                        """, (assigned_doctor_id, patient_id, folder))
                    else:
                        # Insert new assignment
                        cur.execute("""
                            INSERT INTO radiology_folder_assignments 
                            (patient_id, folder, assigned_doctor_id) 
                            VALUES (%s, %s, %s)
                        """, (patient_id, folder, assigned_doctor_id))
                    mysql.connection.commit()
                    ...
                    doctor_name = f"{doctor_info[0]} {doctor_info[1]}" if doctor_info and doctor_info[0] else "Doctor"
                    
                    # Create notification for doctor - store folder name in message for parsing
                    # Format: message contains folder name in quotes for easy extraction
                    create_doctor_notification(
                        doctor_id=assigned_doctor_id,
                        patient_id=patient_id,
                        notification_type='radiology_upload',
                        title='New Radiology Folder Uploaded',
                        message=f'Patient {patient_name} has uploaded radiology files in folder "{folder}".',
                        resource_type=f'radiology_folder:{folder}',  # Store folder name in resource_type
                        resource_id=patient_id
                    )
                    
                    flash(f'Doctor {doctor_name} has been assigned and notified.', 'success')
                except Exception as e:
                    app.logger.error(f"Error assigning doctor: {e}")
                    flash('Error assigning doctor. Please try again.', 'danger')
        ...
        if request.method == 'POST':
            ...
            for file in files:
                ...
                if not os.path.exists(stored_path):
                    app.logger.error(f"File not created at {stored_path}, attempting fallback copy")
                    try:
                        import shutil
                        shutil.copyfile(temp_path, stored_path)
                    except Exception as copy_error:
                        app.logger.error(f"Failed to create file: {copy_error}")
                        flash(f'Failed to save file: {safe_name}', 'danger')
                        continue
                
                try:
                    # Analyze if image - only analyze if file exists and is an image
                    tumor_detected = None
                    confidence = None
                    analysis_date = None
                    if ext in image_extensions and os.path.exists(stored_path):
                        try:
                            result, conf = analyze_mri_image(stored_path)
                            tumor_detected = result
                            confidence = conf
        ...
        doctors_list = [
            {
                'id': d[0],
                'name': f"{d[1]} {d[2]}" if d[1] and d[2] else 'Doctor',
                'specialization': d[3] or ''
            } for d in doctors
        ]
        
        # Get currently assigned doctor for this folder
        assigned_doctor = None
        try:
            cur.execute("""
                SELECT assigned_doctor_id, 
                       CONCAT(u.first_name, ' ', u.last_name) as doctor_name
                FROM radiology_folder_assignments rfa
                JOIN users u ON rfa.assigned_doctor_id = u.id
                WHERE rfa.patient_id = %s AND rfa.folder = %s
            """, (patient_id, folder))
            assignment = cur.fetchone()
            if assignment:
                assigned_doctor = {'id': assignment[0], 'name': assignment[1]}
        except Exception:
            pass  # Table might not exist yet

        return render_template('patient_radiology.html', files=files, folder=folder, doctors=doctors_list, assigned_doctor=assigned_doctor)
    except Exception as e:
...
def view_radiology(file_id):
    ...
    try:
        ...
        user_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'radiology', str(patient_id))
        # stored_name may already include folder path (e.g., "folder_name/filename.jpg")
        # If it includes path separators, use it directly, otherwise join with user_dir
        if os.sep in stored_name or '/' in stored_name:
            # stored_name already includes the folder path
            path = os.path.join(user_dir, stored_name)
        else:
            # stored_name is just the filename, try to find it
            path = os.path.join(user_dir, stored_name)
            if not os.path.exists(path):
                # Try to find it in subdirectories (folders)
        ...
        if not os.path.exists(path):
            app.logger.error(f"File not found: {path} (stored_name: {stored_name}, user_dir: {user_dir})")
        ...
        # For all file types (PDFs, images, etc.), return the file with inline content-disposition
        # This allows the browser to display PDFs in its native viewer and images directly
        ...
        try:
            ...
            # For PDFs, ensure proper headers for browser PDF viewer
            if (file_type or '').lower() == 'pdf':
        ...
        records_list = [
            {
                ...
                'on_chain': record[7] is not None,
                'blockchain_record_id': record[7],
                'blockchain_tx': record[8]
            } for record in records
        ]
...
def doctor_dashboard():
    ...
    try:
        ...
        # Get list of all patients for medical record form — include wallet
        cur.execute("""
            SELECT DISTINCT p.id, CONCAT(p.first_name, ' ', p.last_name) as patient_name, p.wallet_address
            FROM appointments a
            JOIN users p ON a.patient_id = p.id
            WHERE a.doctor_id = %s
        ...
        unread_notification_count = get_unread_doctor_notification_count(doctor_id)
...
def create_notification(patient_id, doctor_id, notification_type, title, message, resource_type=None, resource_id=None):
    """Create a notification for a patient"""
    cur = mysql.connection.cursor()
    try:
        cur.execute("""
            INSERT INTO notifications 
            (patient_id, doctor_id, notification_type, title, message, related_resource_type, related_resource_id)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (patient_id, doctor_id, notification_type, title, message, resource_type, resource_id))
        mysql.connection.commit()
        return True
    except Exception as e:
        mysql.connection.rollback()
...
def create_doctor_notification(doctor_id, patient_id, notification_type, title, message, resource_type=None, resource_id=None):
    """Create a notification for a doctor"""
    cur = mysql.connection.cursor()
    try:
        # Ensure doctor_notifications table exists
        cur.execute("""
            CREATE TABLE IF NOT EXISTS doctor_notifications (
                id INT AUTO_INCREMENT PRIMARY KEY,
                doctor_id INT NOT NULL,
                patient_id INT NOT NULL,
                notification_type ENUM('radiology_upload','appointment','prescription','medical_record','message') NOT NULL DEFAULT 'radiology_upload',
                title VARCHAR(255) NOT NULL,
                message TEXT NOT NULL,
                related_resource_type VARCHAR(50) DEFAULT NULL,
                related_resource_id INT DEFAULT NULL,
                is_read TINYINT(1) DEFAULT 0,
                read_at TIMESTAMP NULL DEFAULT NULL,
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (doctor_id) REFERENCES users(id) ON DELETE CASCADE,
                FOREIGN KEY (patient_id) REFERENCES users(id) ON DELETE CASCADE,
                INDEX idx_doctor_notifications_doctor (doctor_id),
                INDEX idx_doctor_notifications_read (doctor_id, is_read),
                INDEX idx_doctor_notifications_created (created_at)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
        """)
        mysql.connection.commit()
        ...
        cur.execute("""
            ...
            (doctor_id, patient_id, notification_type, title, message, related_resource_type, related_resource_id)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (doctor_id, patient_id, notification_type, title, message, resource_type, resource_id))
        mysql.connection.commit()
        return True
    except Exception as e:
        mysql.connection.rollback()
        app.logger.error(f"Error creating doctor notification: {e}")
        return False
    finally:
        cur.close()

def get_unread_doctor_notification_count(doctor_id):
    """Get count of unread notifications for a doctor"""
    cur = mysql.connection.cursor()
    try:
        cur.execute("SELECT COUNT(*) FROM doctor_notifications WHERE doctor_id = %s AND is_read = 0", (doctor_id,))
        count = cur.fetchone()[0]
        return count
    except Exception as e:
        app.logger.error(f"Error getting doctor notification count: {e}")
        return 0
    finally:
        cur.close()

def get_unread_notification_count(patient_id):
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS notifications (
                ...
                INDEX idx_notifications_read (patient_id, is_read)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
        """)
        mysql.connection.commit()
        
        cur.execute("SELECT COUNT(*) FROM notifications WHERE patient_id = %s AND is_read = 0", (patient_id,))
        count = cur.fetchone()[0]
        return count
    except Exception as e:
...
@login_required
def doctor_patients():
    """Show list of all patients assigned to the doctor"""
    if session.get('role') != 'doctor':
        flash('Access denied. Doctor role required.', 'danger')
        return redirect(url_for('home'))
    ...
    try:
        ...
        if not doctor:
            ...
            return redirect(url_for('logout'))
        doctor_id = doctor[0]
        doctor_name = f"{doctor[1]} {doctor[2]}"
        ...
        cur.execute("""
            SELECT DISTINCT 
                ...
                COUNT(DISTINCT rf.id) as radiology_file_count,
                MAX(a.appointment_date) as last_appointment
            FROM users p
            LEFT JOIN doctor_patient_assignments dpa ON p.id = dpa.patient_id AND dpa.doctor_id = %s AND dpa.status = 'active'
            LEFT JOIN appointments a ON p.id = a.patient_id AND a.doctor_id = %s
            LEFT JOIN medical_records mr ON p.id = mr.patient_id AND mr.doctor_id = %s
            LEFT JOIN radiology_files rf ON p.id = rf.patient_id
            WHERE (dpa.patient_id IS NOT NULL OR EXISTS (
                SELECT 1 FROM appointments a2 WHERE a2.patient_id = p.id AND a2.doctor_id = %s
            ))
            AND p.role = 'patient'
            GROUP BY p.id, p.username, p.email, p.first_name, p.last_name, p.phone, p.date_of_birth, p.gender, p.blood_type
            ORDER BY p.last_name, p.first_name
        ...
        for patient in patients:
            patients_list.append({
                ...
                'first_name': patient[3],
                'last_name': patient[4],
                'full_name': f"{patient[3]} {patient[4]}",
                'phone': patient[5],
                'date_of_birth': patient[6],
                'gender': patient[7],
                'blood_type': patient[8],
                'appointment_count': patient[9] or 0,
                'medical_record_count': patient[10] or 0,
                'radiology_file_count': patient[11] or 0,
                'last_appointment': patient[12]
            })
        ...
        app.logger.error(f"Error in doctor patients list: {e}")
        app.logger.error(traceback.format_exc())
        flash('An error occurred while loading patients.', 'danger')
        return redirect(url_for('doctor_dashboard'))
    finally:
        cur.close()
...
def doctor_patient_detail(patient_id):
    """Doctor view of patient details: medical records and radiology folders"""
    if session.get('role') != 'doctor':
        flash('Access denied. Doctor role required.', 'danger')
        return redirect(url_for('home'))
    ...
    try:
        ...
        has_direct_assignment = cur.fetchone()[0] > 0
        
        # Also check if patient has appointments with this doctor
        cur.execute("""
            SELECT COUNT(*) FROM appointments
            WHERE doctor_id = %s AND patient_id = %s
        """, (doctor_id, patient_id))
        
        has_appointments = cur.fetchone()[0] > 0
        
        if not has_direct_assignment and not has_appointments:
        ...
        # Get medical records
        cur.execute("""
            SELECT id, record_date, diagnosis, treatment, notes, follow_up_date,
        ...
        for record in medical_records:
            records_list.append({
                ...
                'treatment': record[3],
                'notes': record[4],
                'follow_up_date': record[5],
                'signed': bool(record[6])
            })
        
        # Get radiology folders
        cur.execute("""
        ...
        for folder in folders:
            folder_name = folder[0] if folder[0] else 'Default'
            folders_list.append({
                'name': folder_name,
                'file_count': folder[1]
            })
        
        # Create notification that doctor accessed patient records
        create_notification(
            patient_id=patient_id,
            doctor_id=doctor_id,
            notification_type='doctor_access',
            title='Your Records Were Accessed',
            message=f'Dr. {doctor_name} has reviewed your medical records and radiology files.',
        ...
        return render_template('doctor_patient_detail.html',
                             ...
                             radiology_folders=folders_list,
                             doctor_name=doctor_name)
    except Exception as e:
        app.logger.error(f"Error in doctor patient detail: {e}")
        app.logger.error(traceback.format_exc())
        flash('An error occurred while loading patient details.', 'danger')
        return redirect(url_for('doctor_patients'))
    finally:
        cur.close()

@app.route('/doctor/patient/<int:patient_id>/radiology/<path:folder>')
@login_required
def doctor_view_patient_radiology(patient_id, folder):
    """Doctor view of patient radiology files in a specific folder"""
    if session.get('role') != 'doctor':
        flash('Access denied. Doctor role required.', 'danger')
        return redirect(url_for('home'))
    ...
    cur = mysql.connection.cursor()
    try:
        # Get doctor ID and verify access
        cur.execute("SELECT id FROM users WHERE username = %s", (session['username'],))
        doctor = cur.fetchone()
        if not doctor:
            return redirect(url_for('logout'))
        ...
        if cur.fetchone()[0] == 0:
            cur.execute("SELECT COUNT(*) FROM appointments WHERE doctor_id = %s AND patient_id = %s", 
                       (doctor_id, patient_id))
            if cur.fetchone()[0] == 0:
                flash('Access denied.', 'danger')
                return redirect(url_for('doctor_patients'))
        ...
        # Get patient name
        cur.execute("SELECT first_name, last_name FROM users WHERE id = %s", (patient_id,))
        patient_name = cur.fetchone()
        patient_display = f"{patient_name[0]} {patient_name[1]}" if patient_name else "Patient"
        
        # Get files in folder
        if folder == 'Default' or folder == '':
            cur.execute("""
                SELECT id, original_filename, stored_filename, file_type, 
                       tumor_detected, confidence, analysis_date, uploaded_at
                FROM radiology_files
                WHERE patient_id = %s AND (folder IS NULL OR folder = '')
                ORDER BY uploaded_at DESC
            """, (patient_id,))
        else:
            cur.execute("""
        ...
        for file in files:
            files_list.append({
                ...
                'file_type': file[3],
                'tumor_detected': file[4],
                'confidence': float(file[5]) if file[5] else None,
                'analysis_date': file[6],
                'uploaded_at': file[7]
            })
        ...
        create_notification(
            ...
            doctor_id=doctor_id,
            notification_type='doctor_access',
            title='Radiology Files Viewed',
            message=f'Your radiology files in folder "{folder}" were reviewed by your doctor.',
            resource_type='radiology_folder',
            resource_id=patient_id
        )
        
        return render_template('doctor_patient_radiology.html',
                             patient_id=patient_id,
                             patient_name=patient_display,
                             folder=folder if folder != 'Default' else '',
                             files=files_list)
    except Exception as e:
        app.logger.error(f"Error viewing patient radiology: {e}")
        flash('An error occurred.', 'danger')
        return redirect(url_for('doctor_patient_detail', patient_id=patient_id))
    finally:
        cur.close()

@app.route('/doctor/patient/<int:patient_id>/radiology/view/<int:file_id>')
@login_required
def doctor_view_radiology_file(patient_id, file_id):
    """Doctor view of patient radiology file (image or PDF)"""
    if session.get('role') != 'doctor':
        flash('Access denied.', 'danger')
        return redirect(url_for('home'))
    ...
    try:
        ...
        stored_name, original_name, file_type = row
        user_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'radiology', str(patient_id))
        path = os.path.join(user_dir, stored_name)
        ...
        mimetype = mime_types.get((file_type or '').lower(), 'application/octet-stream')
        ...
        app.logger.error(f"Error viewing radiology file: {e}")
        flash('An error occurred while viewing the file.', 'danger')
        return redirect(url_for('doctor_patient_detail', patient_id=patient_id))
    finally:
        cur.close()

@app.route('/doctor/patient/<int:patient_id>/radiology/download/<int:file_id>')
@login_required
def doctor_download_radiology_file(patient_id, file_id):
    """Doctor download of patient radiology file"""
    if session.get('role') != 'doctor':
        flash('Access denied.', 'danger')
        return redirect(url_for('home'))
    ...
    try:
        ...
        app.logger.error(f"Error downloading radiology file: {e}")
        flash('An error occurred while downloading the file.', 'danger')
        return redirect(url_for('doctor_patient_detail', patient_id=patient_id))
    finally:
        cur.close()
...
def patient_notifications():
    """View all notifications for the logged-in patient"""
    if session.get('role') != 'patient':
        flash('Access denied. Patient role required.', 'danger')
        return redirect(url_for('home'))
    ...
    try:
        ...
        cur.execute("""
            SELECT n.id, n.title, n.message, n.notification_type, n.is_read,
                   n.created_at, n.related_resource_type, n.related_resource_id,
        ...
        for notif in notifications:
            notifications_list.append({
                ...
                'created_at': notif[5],
                'resource_type': notif[6],
                'resource_id': notif[7],
                'doctor_name': notif[8] or 'System'
            })
        
        # Get unread count
        unread_count = get_unread_notification_count(patient_id)
...
def get_unread_notifications_count():
    ...
    try:
        ...
        count = get_unread_notification_count(patient[0])
...
@app.route('/doctor/notifications/unread-count')
@login_required
def get_unread_doctor_notifications_count():
    """Get unread doctor notification count (for AJAX)"""
    if session.get('role') != 'doctor':
        return jsonify({'count': 0})
    ...
    try:
        ...
        count = get_unread_doctor_notification_count(doctor[0])
        return jsonify({'count': count})
    except Exception as e:
        app.logger.error(f"Error getting doctor unread count: {e}")
        return jsonify({'count': 0})
    finally:
        cur.close()
...
def doctor_notifications():
    """View all notifications for the logged-in doctor"""
    if session.get('role') != 'doctor':
        flash('Access denied. Doctor role required.', 'danger')
        return redirect(url_for('home'))
    ...
    try:
        ...
        cur.execute("""
            ...
            LEFT JOIN users p ON n.patient_id = p.id
            WHERE n.doctor_id = %s
            ORDER BY n.created_at DESC
            LIMIT 50
        """, (doctor_id,))
        ...
        for notif in notifications:
            notifications_list.append({
                ...
                'resource_type': notif[6],
                'resource_id': notif[7],
                'patient_name': notif[8] or 'Patient',
                'patient_id': notif[9]
            })
        
        # Get unread count
        unread_count = get_unread_doctor_notification_count(doctor_id)
        
        return render_template('doctor_notifications.html',
                             notifications=notifications_list,
                             unread_count=unread_count)
    except Exception as e:
        app.logger.error(f"Error loading doctor notifications: {e}")
        flash('An error occurred while loading notifications.', 'danger')
        return redirect(url_for('doctor_dashboard'))
    finally:
        cur.close()
...
def mark_doctor_notification_read(notification_id):
    ...
    try:
        ...
        mysql.connection.rollback()
        app.logger.error(f"Error marking doctor notification read: {e}")
        return jsonify({'error': 'Failed to update notification'}), 500
    finally:
        cur.close()
...
@app.route('/doctor/add-medical-record-api', methods=['POST'])
@login_required
def add_medical_record_api():
    """API: Save record to DB and return JSON for MetaMask anchoring."""
    if session.get('role') != 'doctor':
        return jsonify({'success': False, 'error': 'Doctor role required.'}), 403
    ...
    try:
        # Get doctor info
        cur.execute("SELECT id, wallet_address FROM users WHERE username = %s", (session['username'],))
        doctor = cur.fetchone()
        if not doctor:
            return jsonify({'success': False, 'error': 'Doctor not found.'}), 404
        doctor_id, doctor_wallet = doctor

        # Get patient wallet
        cur.execute("SELECT wallet_address FROM users WHERE id = %s", (patient_id,))
        patient_row = cur.fetchone()
        patient_wallet = patient_row[0] if patient_row else None

        if not patient_wallet:
            return jsonify({'success': False, 'error': 'Patient has no wallet address.'}), 400

        # Compute hash and save to DB
        record_hash = compute_record_hash(int(patient_id), doctor_id, diagnosis, treatment or '')

        cur.execute("""
            INSERT INTO medical_records (patient_id, doctor_id, diagnosis, treatment, record_date, record_hash)
            VALUES (%s, %s, %s, %s, NOW(), %s)
        """, (patient_id, doctor_id, diagnosis, treatment, record_hash))
        mysql.connection.commit()
        ...
        try:
            ...
            if not private_key_pem:
                ...
                cur.execute(
                    ...
                    (private_pem, public_pem, doctor_id)
                )
                private_key_pem = private_pem

            if private_key_pem:
                signature_bytes = sign_document_util(
                    record_hash.encode('utf-8'), private_key_pem)
                cur.execute("""
                    UPDATE medical_records
                    SET signature = %s, signature_time = CURRENT_TIMESTAMP, signed_by = %s
                    WHERE id = %s
                """, (signature_bytes, doctor_id, record_id))
                signed = True
        except Exception as e:
            app.logger.warning(f"RSA signing failed for record {record_id}: {e}")

        mysql.connection.commit()

        log_action(session['username'], 'add_medical_record',
                   f'Added record #{record_id} for patient #{patient_id} (signed={signed}, MetaMask flow)')
        ...
        app.logger.error(f"Error in add_medical_record_api: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        cur.close()


@app.route('/doctor/update-record-blockchain', methods=['POST'])
@login_required
def update_record_blockchain():
    """API: Update a medical record with blockchain info after MetaMask TX."""
    if session.get('role') != 'doctor':
        return jsonify({'success': False, 'error': 'Doctor role required.'}), 403

    data = request.get_json()
    record_id = data.get('record_id')
    blockchain_record_id = data.get('blockchain_record_id')
    ...
    try:
        cur.execute("""
            UPDATE medical_records
            SET blockchain_record_id = %s, blockchain_tx = %s
            WHERE id = %s
        """, (blockchain_record_id, blockchain_tx, record_id))
        mysql.connection.commit()

        log_action(session['username'], 'blockchain_anchor',
                   f'Record #{record_id} anchored on-chain. TX: {blockchain_tx}')

        return jsonify({'success': True})

    except Exception as e:
        app.logger.error(f"Error updating record blockchain info: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        cur.close()
...
def add_medical_record():
    ...
    cur = mysql.connection.cursor()
    try:
        # 1. Get doctor id + wallet
        cur.execute("SELECT id, wallet_address FROM users WHERE username = %s",
                    (session['username'],))
        doctor = cur.fetchone()
        if not doctor:
            flash('Doctor record not found.', 'danger')
        ...
        cur.execute("SELECT wallet_address FROM users WHERE id = %s", (patient_id,))
        patient_row = cur.fetchone()
        if not patient_row or not patient_row[0]:
            flash('Patient has no wallet address registered.', 'danger')
            return redirect(url_for('doctor_dashboard'))
        patient_wallet = patient_row[0]

        # 3. Insert record to MySQL — always compute hash
        record_hash = compute_record_hash(int(patient_id), doctor_id, diagnosis, treatment or '')
        ...
        # 4. Push to blockchain using doctor's wallet as signer
        blockchain_record_id = None
        blockchain_tx        = None
        try:
            from blockchain_utils import add_record_onchain, get_key_for_wallet
            doctor_key = get_key_for_wallet(doctor_wallet)

            if not doctor_key:
                app.logger.warning(
                    f"No private key found for doctor wallet {doctor_wallet} — skipping anchor")
                flash('Record saved with hash. Blockchain anchoring skipped: doctor key not found.', 'warning')
            else:
                _, blockchain_record_id, blockchain_tx = add_record_onchain(
                    patient_wallet, doctor_wallet, doctor_key,
                    diagnosis, treatment, int(patient_id), doctor_id
                )
                cur.execute(
                    """
                    UPDATE medical_records
                    SET blockchain_record_id = %s,
                        blockchain_tx = %s
                    WHERE id = %s
                    """,
                    (blockchain_record_id, blockchain_tx, record_id)
                ...
                app.logger.info(
                    f"Record {record_id} anchored — "
                    f"chain_id={blockchain_record_id} tx={blockchain_tx}"
                )
        except Exception as e:
            app.logger.warning(f"Blockchain anchoring failed for record {record_id}: {e}")
            flash('Record saved to database with hash but blockchain anchoring failed.', 'warning')
        ...
        if action == 'sign_and_add':
            ...
            if not private_key_pem:
                try:
                    ...
                    app.logger.error(f"Failed to generate RSA keys: {e}")
                    flash('Could not create signing keys.', 'danger')
                    mysql.connection.commit()
                    return redirect(url_for('doctor_dashboard'))

            if private_key_pem:
                payload     = f"{patient_id}|{doctor_id}|{diagnosis}|{treatment}"
                hash_to_sign = (record_hash or
                                hashlib.sha256(payload.encode()).hexdigest())
                try:
                    signature_bytes = sign_document_util(
                        hash_to_sign.encode('utf-8'), private_key_pem)
                    cur.execute(
                        """
                        UPDATE medical_records
                        SET signature = %s,
                    ...
                    flash('Record saved, signed, and anchored on blockchain.', 'success')
                except Exception as e:
                    app.logger.error(f"Signing failed for record {record_id}: {e}")
...
@app.route('/blockchain/record/<int:record_id>')
@login_required
def verify_blockchain_record(record_id):
    cur = mysql.connection.cursor()
    try:
        cur.execute("""
            SELECT mr.id, mr.diagnosis, mr.record_hash, 
                   mr.blockchain_record_id, mr.blockchain_tx,
                   mr.record_date,
                   CONCAT(p.first_name,' ',p.last_name) as patient_name,
                   CONCAT(d.first_name,' ',d.last_name) as doctor_name
            FROM medical_records mr
        ...
        result = {
            ...
            'diagnosis': row[1],
            'record_hash': row[2],
            'blockchain_record_id': row[3],
            'blockchain_tx': row[4],
            'record_date': str(row[5]),
            'patient': row[6],
            'doctor': row[7],
            'on_chain': False
        }
        ...
        if row[3]:
            try:
                from blockchain_utils import contract
                chain_data = contract.functions.getRecordMetadata(
                    int(row[3])
                ).call()
                result['on_chain'] = True
                result['chain_doctor'] = chain_data[1]
                result['chain_patient'] = chain_data[2]
                result['chain_timestamp'] = chain_data[5]
            except Exception as e:
